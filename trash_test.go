...
func TestListPackages(t *testing.T) {
	assert := require.New(t)
	p := listPackages("github.com/juandavidcruzgomez/trash", "vendor")
	logrus.Debug(p)
	assert.Equal(3, len(p))
	assert.Contains(p, "github.com/juandavidcruzgomez/trash")
	assert.Contains(p, "github.com/juandavidcruzgomez/trash/util")
	assert.Contains(p, "github.com/juandavidcruzgomez/trash/conf")
}
